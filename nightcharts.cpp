...
int Nightcharts::draw(QPainter *painter)
{
    ...
    else if (this->ctype==Nightcharts::Histogramm)
    {
        ...
        for (int i=1;i<10;i++)
        {
            painter->drawLine(cX-3,cY+cH/10*i,cX+3,cY+cH/10*i);    //\E4\E5\EB\E5\ED\E8\FF \EF\EE \EE\F1\E8 Y
            //painter->drawText(cX-20,cY+cH/10*i,QString::number((10-i)*10)+"%");
        }
        painter->drawLine(cX,cY+cH,cX,cY);         //\EE\F1\FC Y
        painter->drawLine(cX,cY,cX+4,cY+10);       //\F1\F2\F0\E5\EB\EA\E8
        painter->drawLine(cX,cY,cX-4,cY+10);
        painter->drawLine(cX,cY+cH,cX+cW,cY+cH);   //\EE\F1\FC \D5
